...
- **You confirm or correct the report.** This is the only conversational step, and
  it is deliberate: a wrong architecture discovery poisons every covenant decision
...
1. **Layer names match reality.** If it says "services layer" and your repo calls
   it `lib/`, correct it now — every future covenant check depends on this.
...
3. Commit the governance files on your setup branch:
   `git add CLAUDE.md .claude/settings.json .claude/baseline.json .claude/covenant_integrity.sha256 .claude/hooks/ .claude/commands/ .githooks/ .github/workflows/covenant.yml .mcp.json quarantine.txt .team_aliases .gitignore`
...
4. **Commit + push.** Say `commit and push`. The model runs audit → review →
   tiered tests and prints a COVENANT REPORT (loud SKIPs on unchanged parts). It then
   states the branch + remote and waits for your `yes` (Covenant Step 4). The
...
4. **Commit + push.** Say `commit and push`. Same COVENANT REPORT, same hooks. The
   difference: **the `git push` permission prompt is a UI approval card**, not a
   terminal y/n, and there is no `cc-push` "type the branch" step — so the **Covenant
   Step 4 chat confirmation is your primary human covenant.** Actually read the
...
| Signal | Expected |
|---|---|
| First covenant run | Passes despite existing debt (baseline absorbs it by identity) |
| Small change after a push | Covenant completes in seconds with loud script-emitted SKIPs |
