#!/usr/bin/env bash
# uninstall.sh — Remove every trace of the Covenant governance framework
#
# Usage: ./uninstall.sh
#
# Removes (local, per-repo):
#   • Dev guide and implementation package files copied to repo root
#   • .githooks/covenant.sh, pre-commit, pre-push, verify_governance_integrity.sh
...
#   • .mcp.json
#   • .claude/ directory (covenant_state.json, baseline.json, session_state.json,
#                         session_spend.tmp, git_cache.json, graph.pid,
#                         checkpoints/, commands/, hooks/, checkpoint_tool.py,
#                         covenant_integrity.sha256, settings.json)
...
_check_framework_staleness() {
    ...
    #
    # COVENANT_SKIP_STALENESS_CHECK=1 skips this entirely — see
...
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo " Covenant Governance Framework — Uninstaller"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
# Guard: refuse to run against the Covenant repo itself
...
# Detect via ANY known install artifact, not covenant_state.json alone. That file
...
_governed_repo_detected() {
    [ -f ".claude/covenant_state.json" ] && return 0
    [ -f ".githooks/covenant.sh" ] && return 0
    ...
    [ -f ".github/workflows/covenant.yml" ] && grep -q "covenant\.sh" ".github/workflows/covenant.yml" 2>/dev/null && return 0
    [ -f ".mcp.json" ] && grep -q "code-review-graph" ".mcp.json" 2>/dev/null && return 0
    return 1
}
_governed_repo_detected || _error "No governed repo detected (checked .claude/covenant_state.json, .githooks/covenant.sh, .claude/hooks/pre_bash_trust_root_guard.sh, .claude/checkpoint_tool.py, .claude/settings.json, .github/workflows/covenant.yml, .mcp.json — none found). Nothing to remove."
...
for f in ".githooks/covenant.sh" ".githooks/pre-commit" ".githooks/pre-push" ".githooks/verify_governance_integrity.sh"; do
    [ -f "$f" ] && LOCAL_FILES+=("$f")
done

# CI workflow — only if it contains covenant.sh references
if [ -f ".github/workflows/covenant.yml" ] && grep -q "covenant\.sh" ".github/workflows/covenant.yml" 2>/dev/null; then
    LOCAL_FILES+=(".github/workflows/covenant.yml")
fi
...
echo "  git add -u"
echo "  git commit -m 'chore: remove Covenant governance framework'"
echo ""
